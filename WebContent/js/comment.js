...
jQuery.fn.rater = function(options) {
    ...
    for (var value = settings.min; value <= settings.max; value += settings.step) {
        ...
        item.css('background-image', 'url(' + settings.image + ')');
        item.addClass('rater-star-happy');
    ...
    content.find('.rater-star-item').mouseover(function() {
        ...
        if (typeof settings.title_format == 'function') {   
            var selectedValue = jQuery(this).parents().nextAll('.rater-star-result').attr("selectedValue");
            if (selectedValue)
                jQuery(this).parents().nextAll('.rater-star-result').html(selectedValue);
            else
                jQuery(this).parents().nextAll('.rater-star-result').html("<font color=\"red\">请点击星星评分</font>");
        }
    }).mouseover(function() {
        ...
        //显示当前分值
        if (typeof settings.title_format == 'function') {
            jQuery(this).parents().nextAll('.rater-star-result').html(current_value + '星&nbsp;' + settings.title_format(current_value));
        }
    }).click(function() {
        ...
        if (typeof settings.title_format == 'function') {
            jQuery(this).parents().nextAll('.rater-star-result').html(current_value + '星&nbsp;' + settings.title_format(current_value));
            jQuery(this).parents().nextAll('.rater-star-result').attr("selectedValue", current_value + '星&nbsp;' + settings.title_format(current_value));
        }
        $("#StarNum").val(current_value);
        //设置当前值
       jQuery(this).parents().children('.subItemId').val(current_value)
    })
