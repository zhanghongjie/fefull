package com.biz.model.base;

import com.jfinal.plugin.activerecord.Model;
import com.jfinal.plugin.activerecord.IBean;

/**
 * Generated by JFinal, do not modify this file.
 */
@SuppressWarnings("serial")
public abstract class BaseBizNavigator<M extends BaseBizNavigator<M>> extends Model<M> implements IBean {

	public void setPkNavigator(java.lang.Integer pkNavigator) {
		set("PK_NAVIGATOR", pkNavigator);
	}

	public java.lang.Integer getPkNavigator() {
		return get("PK_NAVIGATOR");
	}

	public void setFkMember(java.lang.Integer fkMember) {
		set("FK_MEMBER", fkMember);
	}

	public java.lang.Integer getFkMember() {
		return get("FK_MEMBER");
	}

	public void setFkNavigatorCategory(java.lang.Integer fkNavigatorCategory) {
		set("FK_NAVIGATOR_CATEGORY", fkNavigatorCategory);
	}

	public java.lang.Integer getFkNavigatorCategory() {
		return get("FK_NAVIGATOR_CATEGORY");
	}

	public void setNavigatorName(java.lang.String navigatorName) {
		set("NAVIGATOR_NAME", navigatorName);
	}

	public java.lang.String getNavigatorName() {
		return get("NAVIGATOR_NAME");
	}

	public void setNavigatorUrl(java.lang.String navigatorUrl) {
		set("NAVIGATOR_URL", navigatorUrl);
	}

	public java.lang.String getNavigatorUrl() {
		return get("NAVIGATOR_URL");
	}

	public void setDESCRIPTION(java.lang.String DESCRIPTION) {
		set("DESCRIPTION", DESCRIPTION);
	}

	public java.lang.String getDESCRIPTION() {
		return get("DESCRIPTION");
	}

	public void setCreateById(java.lang.Integer createById) {
		set("CREATE_BY_ID", createById);
	}

	public java.lang.Integer getCreateById() {
		return get("CREATE_BY_ID");
	}

	public void setCreateByTime(java.util.Date createByTime) {
		set("CREATE_BY_TIME", createByTime);
	}

	public java.util.Date getCreateByTime() {
		return get("CREATE_BY_TIME");
	}

	public void setUpdateById(java.lang.Integer updateById) {
		set("UPDATE_BY_ID", updateById);
	}

	public java.lang.Integer getUpdateById() {
		return get("UPDATE_BY_ID");
	}

	public void setUpdateByTime(java.util.Date updateByTime) {
		set("UPDATE_BY_TIME", updateByTime);
	}

	public java.util.Date getUpdateByTime() {
		return get("UPDATE_BY_TIME");
	}

	public void setIsValid(java.lang.Integer isValid) {
		set("IS_VALID", isValid);
	}

	public java.lang.Integer getIsValid() {
		return get("IS_VALID");
	}

}
